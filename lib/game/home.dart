...
import 'package:huofu/api/buildcheck.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  void _reload() {
    buildcheck().then((value) async {
      var data = jsonDecode(value.data);
      PackageInfo packageInfo = await PackageInfo.fromPlatform();
      if ((int.tryParse(packageInfo.buildNumber) ?? double.infinity) <
          data['build']) {
        Navigator.of(context).maybePop();
      }
      Timer(Duration(seconds: 1), _reload);
    }).catchError((error) {
      Timer(Duration(seconds: 1), _reload);
    });
  }
