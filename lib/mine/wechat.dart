...
import 'package:huofu/api/wechatdelete.dart';
import 'package:huofu/api/wechatdetail.dart';
import 'package:huofu/api/wechatedit.dart';
import 'package:huofu/api/wechatupdate.dart';
...
class WeChat extends StatefulWidget {
  @override
  WeChatState createState() => WeChatState();
}

class WeChatState extends State {
  int? id;
  bool isAdd = true;
  bool edit = false;
  String number = '';
  ...
  _onRefresh() {
    ...
    wechatdetail().then((response) {
      setState(() {
        isAdd = response.weixin == null;
        id = response.weixin?.id;
        number = response.weixin?.numbers ?? '';
        image = response.weixin?.images ?? '';
      });
    }).catchError((error) {
      setState(() {
        isAdd = true;
  ...
  onUpload(PickedFile file) async {
    ...
    File? img = await FlutterImageCompress.compressAndGetFile(
        file.path, temp.path + '/wechat.jpg',
        quality: 80);
    if (img != null) {
      loading();
      Response body = await upload(img.path);
            ...
            children: [
              ...
              Container(
                child: GestureDetector(
                  onTap: () async {
                    loading();
                    var data = await wechatdelete(id.toString());
                    toast(data.data['msg'] ?? "");
                    setState(() {
                      isAdd = true;
                      edit = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(isAdd ? '添加微信' : (edit ? '编辑微信' : '微信')),
              ...
              child: IButton2(
                ...
                fieldCallBack: () async {
                  ...
                  if (vcode != null) {
                    ...
                    ResponseModel data;
                    if (isAdd) {
                      data = await wechatedit(number, image, vcode);
                    } else {
                      data = await wechatupdate(
